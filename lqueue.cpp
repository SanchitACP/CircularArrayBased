...
int lq_send (lqd_t __msgid, const char *__msg, size_t __msg_len, unsigned int __msg_prio)
{
	...
	lqueue_table[__msgid].lq_ptr->push ((void *)local_buf);
...
int lq_timedsend(lqd_t mqdes, const char *msg_ptr,
              size_t msg_len, unsigned msg_prio,
              int timeout)
{
	//lqd_t __msgid, const char *__msg, size_t __msg_len, unsigned int __msg_prio
	int i = 0;
	int joesanchittimeout = 1000;
	while (lq_send(mqdes, msg_ptr, msg_len, msg_prio) == -1 && i < timeout)
	{
		usleep(joesanchittimeout);
		i++;
	}
}
